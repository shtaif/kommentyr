...
export default connect(
    ...
    class App extends Component {
        ...
        constructor(props) {
            ...
            this.apollo = new ApolloClient({uri: 'https://localhost:4443/graphql-api'});
            JsTimeAgo.locale(en);
            let me = lockr.get('me');
            this.props.dispatch({type: 'USER_LOADED', payload: me});
        }
        ...
        * either reset comment to the first "page" of comments, or just load the next page and add it to the current comments
        ...
        async signIn(email, password) {
            let { data: { me } } = await this.apollo.mutate({
                mutation: gql`
            ...
            this.props.dispatch({type: 'USER_SIGN_IN', payload: me});
        }


        async signOut() {
            let { data: { result } } = await this.apollo.mutate({mutation: gql`
                mutation {
                    result: signOut
                }
            `});
        ...
        async signUp(email, password) {
            let { data: { user } } = await this.apollo.mutate({
                mutation: gql`
                    mutation ($email: String, $password: String) {
                        user: signUp(
                            email: $email,
                    ...
                    <div className="comments-wrapper padding-sides">
                        ...
                        <div>
                            {this.props.me ? (
                                ...
                                <Fragment>
                                    <button onClick={() => this.signIn('1dorshtief@gmail.com', '12345')}>Sign In</button>
                                    <button onClick={async () => {
                                                await this.signUp('1dorshtief@gmail.com', '12345');
                                            }}>
                                        Sign Up
                                    </button>
                                </Fragment>
                        ...
                        <div className="comment-list"
                            ...
                            <InfiniteScroll initialLoad={false}
                                ...
                                {/*<CommentNodeTree comments={this.state.comments}
                                                 flipMoveProps={{
                                                     typeName: null,
                                                 ...
                                                 itemTemplate={(comment, k) =>
                                                     <Comment className="comment-item"
                                                              ...
                                                              imageOnClick={e => {
                                                                  e.preventDefault();
                                                                  this.openUserDetails(comment.email, comment.emailHash);
                                                              }}
                                                              onClick={() => {
